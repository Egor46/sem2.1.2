...
void taskA(TuringStrip ts) {
	...
	for (int i = 1; i < T; i++) {
		if (q == 1) counter++;
		bool b = process(q, ts); // обрабатываем каждую клетку и записываем, пришло ли нам в конечном счёте стоп значение
		if (b) {
			cout << "Машина находилась в состоянии q1 " << counter << " раз\n" << "программа завершилась в связи с нулевым состоянием"; // если да, то здесь выходим из программы и уточняем условие выхода
			return;
		}

	}
	cout << "Машина находилась в состоянии q1 " << counter << " раз\n" << "программа выполнила все " << T << " тактов"; // теперь надо доделать функцию, которая задана в задании, но это позже
	return;
}
